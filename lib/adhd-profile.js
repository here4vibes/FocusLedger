...
const { chatMessages } = require('./ai-chat');
...
/**
 * One-time bootstrap for existing users whose adhd_profile is still empty {}.
 * Pulls up to 30 days of buddy_conversations and synthesizes an initial profile
 * from their history. Called fire-and-forget via setImmediate — never blocks.
 */
async function bootstrapAdhdProfile(pool, userId) {
  const existing = await getAdhdProfile(pool, userId);
  if (Object.keys(existing).length > 0) return; // Already populated

  const result = await pool.query(
    `SELECT role AS role, message AS content
  ...
  if (result.rows.length < 3) return; // Not enough historical signal

  await updateAdhdProfile(pool, userId, result.rows);
}

module.exports = { getAdhdProfile, updateAdhdProfile, buildProfilePromptAddition, bootstrapAdhdProfile };
