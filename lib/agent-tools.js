...
const TOOL_DEFS = [
  ...
  },
  {
    name: 'create_task',
    description:
      'Create a new task, optionally with a due date and time. Use when the user asks to add a ' +
      'task, remind them to do something, or nudge them about something at a time. FocusLedger ' +
      'pushes a notification when a task with a due date/time is within an hour of due or overdue ' +
      '(plus morning and evening nudges) — so a due-dated task IS the reminder. To "remind me ' +
      'tomorrow evening", create a task with tomorrow\'s date and an evening time like 18:00.',
    input_schema: {
      type: 'object',
      properties: {
        title: { type: 'string', description: 'Short task title, e.g. "Pack for the trip".' },
        due_date: { type: 'string', description: 'Optional due date, formatted YYYY-MM-DD.' },
        due_time: { type: 'string', description: 'Optional due time in 24h HH:MM, e.g. "18:00".' },
      },
      required: ['title'],
    },
  },
...
const TIERS = { reschedule_task: 'auto', mark_task_done: 'auto', create_task: 'auto', draft_and_send_email: 'confirm' };
...
const SCOPES = { reschedule_task: 'app', mark_task_done: 'app', create_task: 'app', draft_and_send_email: 'world' };
  ...
  switch (name) {
    ...
    case 'create_task': {
      ...
      const ins = await pool.query(
        `INSERT INTO tasks (user_id, title, due_date, due_time, priority, is_completed, created_at)
         VALUES ($1, $2, $3::date, $4::time, 'medium', false, NOW())
         RETURNING id, title`,
        [userId, title.slice(0, 200), dueDate, dueTime]
      );
      const t = ins.rows[0];
      const when = dueDate ? ` for ${dueDate}${dueTime ? ' ' + dueTime : ''}` : '';
      const nudge = (dueDate && dueTime) ? ' — I’ll nudge you when it’s due' : '';
      return {
        ok: true,
        result: { task_id: t.id, title: t.title, due_date: dueDate, due_time: dueTime },
        receipt: `Added “${t.title}”${when}${nudge}`,
