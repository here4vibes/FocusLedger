...
const { getLatestCrossDomainInsight } = require('../db/insights');

/**
 * Return active behavior patterns for a user from buddy_patterns table.
 */
async function runPatternDetection(pool, userId, _tz) {
  // The buddy_patterns table in production has columns pattern_data / detected_at /
  // dismissed (not the confidence_score / metadata / surfaced_at / dismissed_at the
  // old code assumed — long-standing schema drift). Nothing writes to this table,
  // so it's effectively dormant; query the real columns (aliased to the shape callers
  // expect) so the endpoint returns gracefully instead of 500ing.
  const { rows } = await pool.query(
    `SELECT id, pattern_type,
            NULL::numeric AS confidence_score,
            NULL::integer AS occurrence_count,
            pattern_data  AS metadata,
            detected_at   AS surfaced_at,
            dismissed     AS dismissed_at
     FROM buddy_patterns
     WHERE user_id = $1 AND dismissed IS NULL
...
async function buildGreetingContext(pool, userId, sessionCount, _tz) {
  ...
  try {
    ...
    console.warn('[buddyPatterns] weekly task count failed | userId:', userId, '|', e.message);
  }

  if (sessionCount === 1) insights.push('Welcome back to FocusLedger!');

  // Inject the AI-generated cross-domain insight if one was produced this week
  try {
    const result = await getLatestCrossDomainInsight(pool, userId);
    if (result.rows.length > 0) {
      insights.push(result.rows[0].insight_text);
    }
  } catch (e) {
    console.warn('[buddyPatterns] cross-domain insight fetch failed | userId:', userId, '|', e.message);
  }
