...
function logEmail(pool, { userId, templateType, to, subject, resendId, success, error }) {
  ...
  // Match the real email_log schema: resend_message_id (not resend_id), status
  // (not a success boolean), and no error column. Drift here silently killed
  // every delivery-audit write.
  pool.query(
    `INSERT INTO email_log (user_id, template_type, to_email, subject, resend_message_id, status)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [userId || null, templateType || null, Array.isArray(to) ? to[0] : to,
     subject || null, resendId || null, success ? 'sent' : (error ? `failed: ${String(error).slice(0, 200)}` : 'failed')]
...
async function sendEmail(poolOrOpts, maybeOpts) {
  ...
  const { to, from, subject, html, text, replyTo, cc, templateType, userId } = opts;
