 ...
 * If the user set a custom hour (anything other than the default), honor it.
 * Otherwise derive the hour from their peak-energy window.
 *
 * @param {number|null|undefined} configuredHour — users.notif_morning_hour
 * @param {string|null|undefined} peakEnergy — adhd_profile.peak_energy
 * @returns {number} hour 0-23
 */
function resolveMorningHour(configuredHour, peakEnergy) {
  const hour = typeof configuredHour === 'number' ? configuredHour : DEFAULT_MORNING_HOUR;
  // User explicitly customized → respect their choice.
  if (hour !== DEFAULT_MORNING_HOUR) return hour;
  // Still on the default → let energy window decide.
