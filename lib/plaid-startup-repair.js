 ...
 * 9-12. UNIQUE indexes on nudges, notification_send_log, push_subscriptions, push_tokens
 * 13. One-time: confirm all unconfirmed non-pending plaid_transactions to expenses
 */
...
module.exports = async function repairPlaidSchema(pool) {
  ...
  try {
    ...
    // 1. Ensure full (non-partial) UNIQUE index on plaid_transactions.transaction_id.
    //    Must be non-partial so ON CONFLICT (transaction_id) without a WHERE clause
    //    can use it for conflict arbitration. PostgreSQL allows multiple NULLs in a
    //    UNIQUE index (NULL ≠ NULL), so no WHERE guard is needed.
    try {
      // Drop any pre-existing partial index so the full one can use the same name.
      ...
      `);
      console.log('[startup] plaid_transactions UNIQUE index ensured (full, non-partial)');
    } catch (e) {
    ...
    // 8. UNIQUE index on plaid_accounts (account_id)
    //    Backs ON CONFLICT (account_id) in upsertPlaidAccount (db/money-prisma.js).
    //    The CREATE TABLE IF NOT EXISTS DDL included UNIQUE but never applied to
    //    pre-existing tables. Without this, upsertPlaidAccount would throw on re-links,
    //    and plaid_accounts could have duplicate rows causing empty accountMaps in sync.
    try {
      await client.query(`
        CREATE UNIQUE INDEX IF NOT EXISTS plaid_accounts_account_id_unique
          ON plaid_accounts (account_id)
      `);
      console.log('[startup] plaid_accounts.account_id UNIQUE index ensured');
    } catch (e) {
      if (!e.message.includes('does not exist')) {
        console.error('[startup] plaid_accounts UNIQUE index failed:', e.message);
      }
    }

    // 9–12. Additional ON CONFLICT targets not covered by earlier startup steps.
    //   Each in its own try/catch — non-fatal if table doesn't exist yet.
    const extraIndexes = [
      // nudges(user_id, notification_key) — lib/nudgeGenerator.js
      `CREATE UNIQUE INDEX IF NOT EXISTS nudges_user_notification_key_unique
         ON nudges (user_id, notification_key)`,
      // notification_send_log(user_id, notification_key, send_date) — db/notifications.js
      `CREATE UNIQUE INDEX IF NOT EXISTS notification_send_log_user_key_date_unique
         ON notification_send_log (user_id, notification_key, send_date)`,
      // push_subscriptions(user_id, endpoint) — routes/notifications.js
      `CREATE UNIQUE INDEX IF NOT EXISTS push_subscriptions_user_endpoint_unique
         ON push_subscriptions (user_id, endpoint)`,
      // push_tokens(user_id, token) — db/push-tokens.js
    ...
    for (const sql of extraIndexes) {
      try {
        ...
        if (!e.message.includes('does not exist')) {
          console.error('[startup] extra index failed:', e.message);
        }
      }
    }
