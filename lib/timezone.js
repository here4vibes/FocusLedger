...
/**
 * Return { date, hour, minute, weekday } in the given IANA timezone.
 * weekday is normalized to 'Mon'/'Tue'/... — followupEmailCheck compares
 * weekday === 'Mon' for the weekly summary; this field was silently missing
 * before, so that email could never fire.
 * @param {string} tz   — IANA zone, e.g. 'America/New_York'
 * @param {Date}   now  — optional, defaults to new Date()
 * @returns {{ date: string, hour: number, minute: number, weekday: string }}
 */
function getLocalDateParts(tz, now = new Date()) {
  ...
  // Some locales render short weekdays with a trailing period — normalize.
  const weekday = String(parts.weekday || '').replace('.', '').slice(0, 3);
...
/**
 * Validate an IANA timezone string. Returns the string if valid, else null.
 * Used by signup/login to safely persist a client-supplied timezone.
 * (Was imported by routes/auth.js but never defined here — every email signup
 * threw "validateTimezone is not a function" before creating the user row.)
 * @param {string} tz
 * @returns {string|null}
 */
...
module.exports = { getLocalDateParts, getUserLocalDate, getUserLocalHour, fetchUserTimezone, fetchUserLocalDate, validateTimezone };
